...
export default function Navbar({ pageType = "Home" }) { 
  ...
  const isShopPageMobile = pageType === "ShopPage";

  // DÜZELTİLDİ: HomePage'in orijinal mobil menü linkleri.
  const homeMobileLinks = ["Home", "Product", "Pricing", "Contact"];

  // DÜZELTİLDİ: Masaüstü menü linkleri (tüm sayfalarda aynı kalabilir)
  const desktopLinks = ["Home", "Shop", "About", "Blog", "Contact", "Pages"];


  // Hangi link listesini kullanacağımızı belirle
  const mobileLinks = isShopPageMobile ? desktopLinks : homeMobileLinks;
      ...
      <Container>
        ...
        <div className="hidden lg:grid h-[72px] grid-cols-[auto_1fr_auto] items-center gap-6">
          ...
          <nav className="min-w-0">
            <ul className="flex justify-center items-center gap-8 whitespace-nowrap">
              {desktopLinks.map((label) => { // Masaüstünde her zaman tam listeyi kullan
        ...
        <div className={`lg:hidden h-[72px] flex items-center justify-between`}>
          ...
          <div className="flex items-center gap-4"> 
            ...
            {/* BU KISIM DÜZELTİLDİ: HomePage'de (veya !ShopPage) sadece 3 ikon + Hamburger görünür */}
            {!isShopPageMobile && (
              <>
                {/* User Icon (Görselde var) */}
            ...
            {/* Hamburger Menu (Her zaman görünür) */}
            <button 
          ...
          <div className="lg:hidden bg-white border-t border-gray-200 py-6 absolute w-full left-0 z-50">
            <nav className="flex flex-col gap-5 text-center px-6"> 
              ...
              {mobileLinks.map((label) => {
                ...
                const linkClasses = [
                  // Ortak stil: BÜYÜK font, Kalın, text-center
              ...
              {/* ShopPage'e özel dikey ikon bloğu - SADECE ShopPage ise gösterilir */}
              {isShopPageMobile && (
                <div className="flex flex-col items-center gap-6 pt-10 px-6">
                  ...
                  {/* Aksiyon İkonları (Dikey Olarak Listeleniyor ve ORTALANIYOR) */}
